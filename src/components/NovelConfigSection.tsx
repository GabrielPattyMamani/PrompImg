...
export default function NovelConfigSection({ novelId, initialConfig, onConfigChange }: Props) {
  ...
  return (
    <div className="mb-3 sm:mb-6 border border-orange-500/30 rounded-xl sm:rounded-2xl overflow-hidden bg-orange-500/5">
      {/* Header — siempre visible */}
      <button
        onClick={() => setOpen(!open)}
        className="w-full px-3 py-2.5 sm:px-4 sm:py-3 flex items-center justify-between hover:bg-orange-500/10 transition-colors"
      >
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-orange-500/20 flex items-center justify-center flex-shrink-0">
            <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-orange-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <svg
          className={`w-4 h-4 sm:w-5 sm:h-5 text-orange-400/50 flex-shrink-0 transition-transform ${open ? 'rotate-180' : ''}`}

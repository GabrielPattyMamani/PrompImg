...
interface Props {
  ...
  orderNum: number
  onClose: () => void
  onUpdated: (ctx: NovelContext) => void
}

export default function EditContextModal({ context, orderNum, onClose, onUpdated }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    onUpdated(data as NovelContext)
    onClose()
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#1a1a22] border border-white/10 rounded-2xl p-6 w-full max-w-2xl shadow-2xl max-h-[90svh] overflow-y-auto">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h2 className="text-white font-semibold text-lg">Editar contexto</h2>
        </div>
